...
2. 归一化方法
    1. min-max标准化方法
        ...
        - 公式: x' = (x-min)/(max-min)
          ![数据归一化处理(min-max标准化方法)](../Data%20Preparation/imgs/Data.Transform.Normalization.png)
        - python实现
          ```python
...
2. 标准化方法
    1. Z-score标准化方法
        - 将数据转换到 0 值附近
        - 公式: x' = (x - 每列均值)/标准差
          ![数据归一化处理(Z-score标准化方法)](../Data%20Preparation/imgs/Data.Transform.Standardization.png)
        - python实现方式
            ```python
            from sklearn import preprocessing
            import numpy as np
            ...
            ```
### 中心化
 - ![数据中心化处理](../Data%20Preparation/imgs/Data.Transform.Center.PCA.png)
1. 使用场景
2. 局限性
3. 案例说明
    1. 中心化
    ...
    # 使用StandardScaler关闭方差缩放，仅做中心化（只减均值，不除以标准差）
    # with_std=False 代表不做标准差归一化，只执行均值中心化
    centered_scaler = preprocessing.StandardScaler(with_mean=True, with_std=False)
    # 填充数据
    centered_scaler.fit(X_train)
    print("各特征均值：", centered_scaler.mean_)
    print("中心化后训练集：")
    print(centered_scaler.transform(X_train))
    
    # 中心化转换测试集（沿用训练集均值，避免测试集信息泄露）
    test_data = np.array([[-1., 1., 0.]])
    print("测试集中心化结果：")
    print(centered_scaler.transform(test_data))
    ```
### One-Hot编码
1. 使用场景:
...
3. 案例说明
    ...
    2. Python 中调用:
        ...
        - 解读:
            ...
            4. 输出结果: 性别[0,1] + 年级[1,0,0] + 爱好[0,1,0,0]
