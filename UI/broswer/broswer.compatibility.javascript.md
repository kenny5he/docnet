...
4. window.location.href问题
    ...
    2. 解决方法: 使用window.location来代替window.location.href.
...
5. frame问题
    ...
    1. 访问frame对象:
        ...
        - 说明: 在IE和Firefox中都可以使用window.document.getElementById(“frameId”)来访问这个frame对象.
    2. 切换frame内容:
...
6. 模态和非模态窗口问题
    1. 说明:
        ...
        - Firefox下则不能.
    2. 解决方法:直接使用window.open(pageURL,name,parameters)方式打开新窗口。
    3. 如果需要将子窗口中的参数传递回父窗口,可以在子窗口中使用window.opener来访问父窗口.
        - 例如:
        ```
...
7. firefox与IE的父元素(parentElement)的区别
    ...
    2. 解决方法: 因为firefox与IE都支持DOM,因此使用obj.parentNode是不错选择.
...
8. document.formName.item(”itemName”) 问题
    ...
    2. 解决方法:统一使用document.formName.elements["elementName"]。
...
10. 自定义属性问题
    ...
    2. 解决方法:统一通过getAttribute() 获取自定义属性。

11. input.type属性问题
    1. 问题说明:
        - IE下input.type属性为只读;但是Firefox下input.type属性为读写。
    2. 解决办法:不修改input.type属性。如果必须要修改,可以先隐藏原来的input,然后在同样的位置再插入一个新的input元素。

12. event.srcElement问题
    1. 问题说明:
        - IE下,even对象有srcElement属性,但是没有target属性;
        - Firefox下,even对象有target属性,但是没有srcElement属性。
    2. 解决方法:使用srcObj = event.srcElement ?event.srcElement : event.target;
...
14. 事件委托方法
    ...
    2. 解决方法:统一使用document.body.onload=new Function(’inject()’);或者document.body.onload = function(){/* 这里是代码 */}
